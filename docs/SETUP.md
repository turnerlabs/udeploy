...
OAUTH_SIGN_OUT_URL={{OKTA_SIGNOUT_URL}}??post_logout_redirect_uri=https:://{{PORTAL_DOMAIN}}
...
### 7. Push Configuration to AWS ### 

Install >= **v0.1.0** [Stash](https://github.com/dabblebox/stash) and run the following commands from the repository root to sync configuration.
...
```bash
$ stash sync infrastructure/portals/prod/.env -s secrets-manager
```

When prompted:
- **context**: udeploy
- **secrets**: multiple
- **kms_key_id**: use the `kms_key_id` from the Terraform output
